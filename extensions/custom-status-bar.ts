...
async function fetchCodexUsage(): Promise<CodexUsage | undefined> {
	...
	const auth = JSON.parse(await readFile(join(home, ".codex", "auth.json"), "utf8"));
	...
	return {
		...
		secondaryPercent: typeof secondary?.used_percent === "number" ? secondary.used_percent : undefined,
...
export default function (pi: ExtensionAPI) {
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					const thinking = thinkingLevel === "off" ? "thinking off" : `thinking ${thinkingLevel}`;
					...
					const contextText = contextUsage
						...
						: "";
					const line1Left =
						theme.fg("accent", `${model} `) + theme.fg("dim", `${contextText} (${thinking})`);
					const line1Right = branch ? theme.fg("success", branch) : theme.fg("dim", "no git branch");
					const line1Pad = " ".repeat(Math.max(1, width - visibleWidth(line1Left) - visibleWidth(line1Right)));

					const line2Left = theme.fg("dim", path);
					const line2Right =
						theme.fg("dim", `↑${formatTokens(inputTokens)} ↓${formatTokens(outputTokens)} $${cost.toFixed(3)}`) +
						(codexUsage
							? theme.fg("dim", "  Codex ") +
								theme.fg(usageColor(codexUsage.primaryPercent), `${codexUsage.primaryPercent}%`) +
								(codexUsage.secondaryPercent === undefined
									? ""
									: theme.fg("dim", "/") +
										theme.fg(usageColor(codexUsage.secondaryPercent), `${codexUsage.secondaryPercent}%`)) +
								(codexUsage.resetAfterSeconds === undefined
									? ""
									: theme.fg("dim", ` reset ${formatDuration(codexUsage.resetAfterSeconds)}`))
							: "");
	...
	pi.registerCommand("custom-footer", {
		...
		handler: async (_args, ctx) => {
			ctx.ui.notify("Restart pi or run /reload to re-enable the custom footer", "info");
		},
	});
}
